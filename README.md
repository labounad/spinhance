...
The production dataset is the **PubChem 8-spin set: 3,126,829 molecules** (~3.13M), regenerated by the Audit-2 spin-equivalence fix, with a leakage-controlled global 10% held-out split (**2,814,147 train / 312,682 test**). On the HPC it lives in the group filesystem at `/gpfs/group/shenvi/Users/labounader/spinsolve/rebuild3M/records_3M{,_train,_test}.json.gz` + stacked spectra shards `rebuild3M/parts/`. (The older 64k ChEMBL set at `mol_to_spin_system/data/spin_systems_chembl.json` is retained for ablations.)
...
The large-corpus fleet streams the stacked PubChem shards. Submit via the Slurm wrappers in `model/scripts/` (e.g. `train_hpc.slurm`), which set the data paths and a reservoir `SAMPLE_N` to pick the tier (64k / 500k / 3M). On the HPC the generated data lives in the group filesystem at `/gpfs/group/shenvi/Users/labounader/spinsolve/` (dataset `rebuild3M/`, checkpoints `runs/` — `model/runs` is a symlink into it). The current sweep is the **025–030 recipe ladder** at two finished tiers — **64k (~10M)** and **500k (~57M)**; the 3M (~137M) tier is **paused / under revision**. The **fleet-best is 500k·029 at 0.0235 ppm** held-out shift MAE — see `model/RESULTS.md` for the full sweep + the test-time refinement results. Score any run on the shared, leakage-controlled held-out split with:
...
| Path | Contents |
|---|---|
| `/gpfs/group/shenvi/Users/labounader/spinsolve/` (HPC) | Group-dir root for all training-generated data (dataset, `runs/` checkpoints, baseline `ckpts/`, spectra) |
| `…/spinsolve/rebuild3M/records_3M{,_train,_test}.json.gz` (HPC) | Production PubChem 8-spin dataset (3.13M) + leakage-controlled split |
| `…/spinsolve/rebuild3M/parts/part_*.npy` (HPC) | Stacked 90 MHz spectra shards (model input) |
