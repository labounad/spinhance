...
## Phase 3+4 EXECUTED (2026-06-03)
- **Full 3M rebuild DONE** — predictor-free array pipeline (screen+fixed-grouping+D-elim → Pretsch shifts + enriched couplings → over-dispersed bake → simulate 90 MHz) over the 3.2M PubChem pool (`/gpfs/home/yimingzhang/hackathon-shared/...pubchem_8spin.csv`). Result: **3,156,290 clean molecules** (~98.6%; rest dropped by the methyl-rotor/deuterium fixes). Parts: `/gpfs/group/shenvi/Users/labounader/spinhance/rebuild3M/parts/part_*.npy` (1605), records `records_3M.json.gz`. Value-reuse fixed (exact-unique: shifts 33.7%, couplings 12.3%, vs old ~0.1%).
- **Leakage-aware global split** — union-find on (matrix fingerprint: sorted deg + shifts@0.1 + |J|@1) AND InChIKey; clusters kept wholly train-or-test. **10% held-out test = 315,629** (cluster-coherent → no near-duplicate leakage; only 1816 multi-clusters, max size 9 — over-dispersion made matrices distinct). `split_istest.npy`/`split_testrank.npy`. Test molecules ranked → **nested** eval subsets (64k⊂500k⊂3M via `test_rank` prefixes).
- **Train/test records** (carry `row`=global index → index the full parts, no spectra duplication; `load_pubchem_records` honors explicit `row`, PR #97): `records_3M_train.json.gz` (2,840,661) + `records_3M_test.json.gz` (315,629, held-out, ranked).
- **Fleet relaunched** (`rebuild3M/train_rebuild.slurm`, GPU): **025/026/027 × {64k, 500k, 3M} = 9 jobs**, reservoir-sampled from the train pool (test excluded). 1M skipped per instruction. All running.
  - NOTE: the **64k tier = a 64k sample of the PubChem-3M train pool** (not the old ChEMBL 64k) — chosen for a standardized data-scaling comparison sharing the nested held-out test. Flag for Lucas.
  - 028/029 recipes available to add. Held-out eval (`records_3M_test` + parts) pending model completion.
