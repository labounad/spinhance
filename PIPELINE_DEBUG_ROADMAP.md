...
- **Full 3M rebuild DONE** — predictor-free array pipeline (screen+fixed-grouping+D-elim → Pretsch shifts + enriched couplings → over-dispersed bake → simulate 90 MHz) over the 3.2M PubChem pool (`/gpfs/home/yimingzhang/hackathon-shared/...pubchem_8spin.csv`). Result: **3,156,290 clean molecules** (~98.6%; rest dropped by the methyl-rotor/deuterium fixes). Parts: `/gpfs/home/labounader/rebuild3M/parts/part_*.npy` (1605), records `records_3M.json.gz`. Value-reuse fixed (exact-unique: shifts 33.7%, couplings 12.3%, vs old ~0.1%).
